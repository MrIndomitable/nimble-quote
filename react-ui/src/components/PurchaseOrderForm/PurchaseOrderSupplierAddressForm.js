...
export const PurchaseOrderSupplierAddressFormComp = ({previousPage, handleSubmit}) => {
  return <form onSubmit={ handleSubmit } className="container">
    ...
    <InputField id="supplier.company" label="Company" type="text" validate={[required(), length({ max: 20 })]}/>
    <InputField id="supplier.contactName" label="Contact name" type="text" validate={[required(), length({ max: 20 })]}/>
    <InputField id="supplier.phone" label="Phone" type="text" validate={[required(), length({ max: 20 })]}/>
    <InputField id="supplier.address" label="Address" type="text" validate={[required(), length({ max: 20 })]}/>
    <InputField id="supplier.state" label="State" type="text"/>
    <InputField id="supplier.country" label="Country" type="text" validate={[required(), length({ max: 20 })]}/>
    <InputField id="supplier.zip" label="Zip Code" type="text" validate={[required(), length({ max: 20 })]}/>
